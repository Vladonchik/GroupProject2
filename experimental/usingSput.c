...
void primeTesting()	{
	...
	//entering new suite.  This should be done for every set of tests.  Choose name arbitrarily inside brackets
	sput_enter_suite("test_prime_true(): All Primes");
	sput_run_test(testPrimeTrue); //running test.  Pass name of your test function without () to run via sput.
	sput_leave_suite(); //when you are going to start a new suite

	sput_enter_suite("testPrimesFalse(): No Primes");	
	sput_run_test(testPrimesFalse);
	sput_leave_suite();
	//when you are done with sput
	sput_finish_testing();
}
...
void testPrimeTrue() {
	...
	while(fscanf(fp,"%d",&number) != EOF) {
		...
		//KEY sput line.  Says that the test will fail UNLESS output from called function is 1.
...
void testPrimesFalse()	{
	...
	while(fscanf(fp,"%d",&number) != EOF) {
		...
		//KEY sput line.  Says that the test will fail UNLESS output from called function is 0.
